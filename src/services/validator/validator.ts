import type { Cell, Dictionary, OpenedContract } from "@ton/ton";
...
import {
  LIQUIDATOR_PRICES_UPDATE_INTERVAL,
  VALIDATOR_MAX_PRICES_ISSUED,
} from "../../steady_config";
import {
  CheckOraclesEnum,
  CheckOraclesMessage,
...
export async function validateBalances(
  ...
  evaaPriceCollector: AbstractCollector,
  bot: Messenger,
  ...
  try {
    ...
    const isPriceDataActual = () =>
      (Date.now() - lastPricesSync) / 1000 < LIQUIDATOR_PRICES_UPDATE_INTERVAL;
    ...
    const updatePrices = async () => {
      ...
      const pricesRes = await retry<PriceData>(
        async () =>
          await evaaPriceCollector.getPrices(evaa.poolConfig.poolAssetsConfig),
        { attempts: 10, attemptInterval: 1000 },
      );
      if (!pricesRes.ok) throw new Error(`Failed to fetch prices`);

      const res = checkPriceData(
        pricesRes.value.dataCell,
        VALIDATOR_MAX_PRICES_ISSUED,
      );
    ...
    const evaaSyncRes = await retry(async () => await evaa.getSync(), {
      attempts: 10,
    ...
    for (const user of users) {
      ...
      if (await db.isTaskExists(user.wallet_address)) {
        logMessage(
          `Validator: Task for ${user.wallet_address} already exists, skipping...`,
        );
        continue;
      }
      ...
      try {
        ...
        logMessage(
          `Failed to calculate heath factor for user ${user.wallet_address}`,
        );
        console.log(e);
        continue;
      }
      ...
      if (!loanAsset || !collateralAsset) {
        logMessage(
          `Failed to select loan or collateral for liquidation: loan id: ${selectedLoanId}, collateral id: ${selectedCollateralId}, skipping user`,
        );
        continue;
      }
      const { totalSupply, totalDebt } = healthParams;
      const { maxLiquidationAmount, maxCollateralRewardAmount } =
        calculateLiquidationAmounts(
          loanAsset,
          collateralAsset,
          totalSupply,
      ...
      if (!assetsConfigDict.has(collateralAsset.assetId)) {
        logMessage(
          `Validator: No config for collateral ${collateralAsset.name}, skipping...`,
        );
        continue;
      }
      ...
      if (!pricesDict.has(collateralAsset.assetId)) {
        logMessage(
          `Validator: No price for collateral ${collateralAsset.name}, skipping...`,
        );
        continue;
      }
      ...
      if (collateralPrice <= 0) {
        logMessage(
          `Validator: Invalid price for collateral ${collateralAsset.name}, skipping...`,
        );
        continue;
      }
      ...
      if (
        minCollateralAmount * collateralPrice >=
        MIN_ALLOWED_COLLATERAL_WORTH * collateralScale
      ) {
        const res = await addLiquidationTask(
          db,
          user,
          loanAsset.assetId,
        ...
        if (!res) {
          await bot.sendMessage(
            `Failed to add db task for user ${user.wallet_address}`,
          );
          // continue;
        } else {
    ...
    if (e.response) {
      logMessage(
        `Validator: Error: ${e.response.status} - ${e.response.statusText}`,
      );
    } else if (e.request) {
